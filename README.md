...
- **主叙述文档**：流程 [docs/fulfillment-flow.md](docs/fulfillment-flow.md)、状态 [docs/state-machine.md](docs/state-machine.md)、迁移 [docs/migration-plan.md](docs/migration-plan.md)。**[docs/project-flow.md](docs/project-flow.md) 仅作历史参考**，新工作以本 README 与 `docs/boundaries.md` 为纲。
...
- 本仓库内：`.cursor/rules/expatth-shared-documents.mdc`（`alwaysApply: true`）概括 **requests / registry / glossary** 与 **project-flow 非主文** 的约定。  
- 在**以父仓库为根**打开时：可将该条 **复制** 到父仓库的 `.cursor/rules/`，或把本目录作子模块后由 Cursor 多根工作区包含本目录；**不要**在应用仓库里再写第二套与 **glossary** 冲突的「主概念」定义。
