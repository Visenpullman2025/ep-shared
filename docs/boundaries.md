...
- 渲染后端返回的展示名，或渲染 shared 明确登记的标准服务配置。
...
- 在页面、弹层或局部 util 里长期维护业务 code 到用户文案的映射。`standardServiceCode`、类目 code、旧 service code 只是标识；用户可见名称必须来自 **StandardService** 合同、后端订单响应，或 shared 已登记配置。临时前端映射只能作为已记录缺口的过渡保护，删除条件必须写入 `api/requests.md`。
