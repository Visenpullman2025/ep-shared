...
在本仓库写 shared 文档、`.cursorrules`、提交说明或向同事说明时，**少用含糊的圈内词与不必要缩写**，改用**谁、做什么、写在哪、什么条件下失败**等可直接核对的说法。凡在本仓库协助编程的助手也应遵守本节（与仓库根 `.cursor/rules/plain-language.mdc` 一致；Cursor 会默认加载该规则）。
